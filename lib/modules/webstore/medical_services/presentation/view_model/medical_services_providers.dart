...
class MedicationRemindersViewModel extends AsyncNotifier<List<MedicationReminderModel>> {
  ...
  Future<List<MedicationReminderModel>> build() async {
    final result = await ref.read(medicalServicesRepositoryProvider).getMedicationReminders();
    return result.when(
      success: (data) => data.map((m) => MedicationReminderModel.fromJson(m)).toList(),
      failure: (_) => [],
    );
  }

  Future<void> addReminder(Map<String, dynamic> data) async {
    final result = await ref.read(medicalServicesRepositoryProvider).createMedicationReminder(data);
    result.when(
      success: (data) {
        final newReminder = MedicationReminderModel.fromJson(data);
        state = AsyncValue.data([newReminder, ...state.value ?? []]);
      },
      failure: (_) {},
    );
  }
  ...
  Future<void> deleteReminder(int id) async {
    ...
    final result = await ref.read(medicalServicesRepositoryProvider).deleteMedicationReminder(id);
    result.when(
      success: (_) {},
      failure: (_) {
  ...
  Future<void> markTaken(int id) async {
    final result = await ref.read(medicalServicesRepositoryProvider).markReminderTaken(id);
    result.when(
      success: (_) {
        ref.invalidateSelf();
      },
